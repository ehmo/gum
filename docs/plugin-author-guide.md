...
| `declared_capabilities.network` | bool | Enforced for hosted plugin subprocesses on the darwin and linux sandbox backends. Any other `GOOS` fails closed: `SandboxedRunner.sandboxedCommand` returns `ErrUnsupportedSandbox` (`pluginenv: OS sandbox unsupported`) and the subprocess never starts. | sandbox spawn error; no dedicated code |
