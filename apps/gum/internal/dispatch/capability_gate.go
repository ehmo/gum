...
// unsupportedCapabilitySuggestion is the spec §5.8 suggestion string, quoted
...
// capabilityGate enforces the spec §5.8 pre-request refusal.
//
...
func capabilityGate(inv *Invocation, rv *ResolvedVariant) *StructuredError {
	...
	// §5.8 makes `unsupported_capabilities` the discriminator for this branch,
...
// partialCapabilityWarning returns the atoms to name in the §5.8 warning
// envelope field, or nil when the variant is not `partial`.
//
// §5.8 keeps a `partial` variant invokable and requires gum.read, gum.write
